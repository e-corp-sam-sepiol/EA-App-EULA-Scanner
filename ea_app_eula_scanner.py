...
def get_installed_ea_games(ea_games_path):
    games = []
    if not os.path.exists(ea_games_path):
        print(f"EA Games path not found: {ea_games_path}")
        return games
    for entry in os.scandir(ea_games_path):
        if entry.is_dir():
            games.append({
                "name": entry.name,
                "path": entry.path
...
# ---------------- MAIN PROCESS ----------------
def main():
    print("Scanning EA App games...")
    games = get_installed_ea_games(EA_GAMES_PATH)
    print(f"Found {len(games)} installed EA App games.")
    results = []
    ...
    for game in tqdm(games, desc="Processing games"):
        ...
        # Choose the "best" EULA for AI analysis
        eula_for_ai = None
    ...
    with open(OUTPUT_FILE, "w", newline='', encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["Game Name", "Install Path", "EULA Found", "Privacy Assessment", "Error/Notes"])
